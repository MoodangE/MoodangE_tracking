...
def construct_dataset(clearml_info_string):
    """Load in a clearml customDataset and fill the internal data_dict with its contents.
    """
    dataset_id = clearml_info_string.replace('clearml://', '')
    dataset = Dataset.get(dataset_id=dataset_id)
    dataset_root_path = Path(dataset.get_local_copy())

    # We'll search for the yaml file definition in the customDataset
    yaml_filenames = list(glob.glob(str(dataset_root_path / "*.yaml")) + glob.glob(str(dataset_root_path / "*.yml")))
    if len(yaml_filenames) > 1:
        raise ValueError('More than one yaml file was found in the customDataset root, cannot determine which one contains '
                         'the customDataset definition this way.')
    elif len(yaml_filenames) == 0:
        raise ValueError('No yaml definition found in customDataset root path, check that there is a correct yaml file '
                         'inside the customDataset root path.')
...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        - Upload customDataset version to ClearML Data if opt.upload_dataset is True
